...
export function Contact() {
  ...
  async function handleSubmit(event) {
    ...
    // Without an access key the form does the honest thing: hand the message
    // to the visitor's own mail client rather than pretend it was sent.
  ...
  return (
    <Section id="contact" label={t.sections.contact}>
      ...
      <Reveal delay={60} className="mt-7 border-t border-line-soft pt-6">
      ...
      >
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-3">
          ...
          <span role="status" aria-live="polite" className="text-[0.875rem]">
            ...
            {!problem && status === 'sent' ? (
              <span className="text-muted">{copy.sent}</span>
            ) : null}
            {!problem && status === 'error' ? (
              <span className="text-accent">
                {copy.error.replace('{email}', profile.email)}
              </span>
            ) : null}
          </span>
        </div>
